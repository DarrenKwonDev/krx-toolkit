...
- [ ] 자동 익절/손절
- [ ] (본절) 진입 가격 대비 호가 잔량이 보유 수량보다 줄어들 경우의 본절 자동 트리거
- [ ] 매수 bet 사이즈 계산
- [ ] 주문 쪼개 넣기
...
- 장시작시간(0s)는 정확하지 않고 동시호가/종가매매 등을 정확히 보내지 않으므로 신뢰하면 안된다
...
- 시간에 의존하는 자동 매매 (ex - 3초 내 미체결시 취소) 기능은 추가하지 않는다. 손매매를 하는 이상 그건 사람이 판단해야 함
